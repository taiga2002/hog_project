...
def roll_dice(num_rolls, dice=six_sided):
    ...
    return 1 if sow_sad else roll_total
# END PROBLEM 1
...
def picky_piggy(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return 7 if score%6 == 0 else int(((10**(score%6))/7)%10)
# END PROBLEM 2
    ...
    def announce(score0, score1):
        ...
        if score != last_score:
            ...
            if gain > running_high:
                high = gain
                print(gain, "point(s)! That's a record gain for Player", str(who)+'!')
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaging(*args):
        score = 0
        for num_rolls in range (trials_count):
            score += original_function(*args)
        return score/trials_count
    return averaging
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    highest_avg, highest_roll = 0, 0
    average = make_averaged(roll_dice, trials_count)
    for num_dice in range (1, 11):
        avg_int =  average(num_dice, dice)
        if highest_avg < avg_int:
            highest_avg, highest_roll = avg_int, num_dice
    return highest_roll
# END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    hog_score = hog_pile(piggy_score+score, opponent_score)
    return num_rolls if piggy_score + hog_score < cutoff else 0  
